...
for type in data_types:
    ...
    for i, (train_index, test_index) in enumerate(kf.split(X)):
        ...
        plt.grid(True)
        plt.savefig(os.path.join(viz_dir,'accuracy', f'Kfold{i}_{type}_accuracy_random_forest_num_trees.png'))
        plt.close()
        ...
        plt.grid(True)
        plt.savefig(os.path.join(viz_dir,'precision', f'Kfold{i}_{type}_precision_random_forest_num_trees.png'))
        plt.close()
        ...
        plt.grid(True)
        plt.savefig(os.path.join(viz_dir,'recall', f'Kfold{i}_{type}_recall_random_forest_num_trees.png'))
        plt.close()
        ...
        plt.savefig(os.path.join(viz_dir,'weights_and_importance', f'Kfold{i}_{type}_rfc_feature_importance.png'))
        plt.close()
        # Save the feature importance data as a CSV file
        importance_rfc_df.to_csv(os.path.join(viz_dir, 'weights_and_importance', f'Kfold{i}_{type}_rfc_feature_importance.csv'), index=False)
